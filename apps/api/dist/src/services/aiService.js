...
exports.generateTwinResponse = generateTwinResponse;
function generateRecommendation(subjects) {
    const recommendations = [];
    subjects.forEach((item) => {
        if (item.mastery < 50) {
            recommendations.push(`Improve ${item.subject}. Your mastery level is ${item.mastery}%. Focus on practice and revision.`);
        }
        else if (item.mastery < 80) {
            recommendations.push(`${item.subject} is improving. Continue practicing to reach advanced level.`);
        }
        else {
            recommendations.push(`Excellent performance in ${item.subject}. Try advanced challenges.`);
        }
    });
    return recommendations;
}
function generateTwinResponse(subjects, selectedSubject, prompt) {
